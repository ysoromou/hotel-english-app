...
// Config par préfixe (REC_, HK_, REST_, SEC_)
const METIER_CONFIG: Record<string, { label: string; icon: React.ReactNode }> = {
...
export default async function MetierPage({
  ...
  actions.forEach(a => {
    ...
    if (p?.statut === 'completed' || p?.statut === 'mastered') completedActions++
  })
  ...
  return (
    <div className="bg-[#F5F5F5] min-h-screen pb-24">
      ...
      {/* En-tête blanc */}
      <div className="bg-white px-5 pt-8 pb-4 sticky top-0 z-20"
      ...
      <div className="px-5 pt-6 max-w-md mx-auto space-y-5">
        ...
        <div className="space-y-3">
          {actions.map((action, index) => {
            const prog   = progressMap.get(action.id)
            const statut = prog?.statut ?? 'not_started'
            ...
            return (
              ...
              >
                {/* Icône module — carré blanc bordure verte */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {isCompleted ? (
                    ...
                    <span className="inline-block px-4 py-1.5 bg-[#006633] text-white text-xs font-bold rounded-full"
